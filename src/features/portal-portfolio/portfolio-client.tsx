...
import {
  Plus, Star, ExternalLink, Copy, Pencil, Trash2, Loader2,
...
function ProjectModal({ open, onClose, onSaved, editProject }: {
  ...
  // project — done during render per React's "adjusting state when a prop
  // changes" pattern, not in an effect.
  const syncKey = open ? (editProject?.id ?? "new") : null
...
export function PortfolioClient() {
  ...
  // Fetch-on-mount/filter-change: load() sets its own loading flag
  // synchronously, which is the point (show a loading state immediately on
  // refetch) — not an accidental cascading render.
