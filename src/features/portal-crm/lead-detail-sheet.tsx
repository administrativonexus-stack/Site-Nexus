...
export function LeadDetailSheet({ lead, open, onClose, onLeadUpdated }: LeadDetailSheetProps) {
  ...
  const [syncedLeadId, setSyncedLeadId] = useState<string | undefined>(undefined)

  // Reset the notes/niche fields whenever a different lead is opened — done
  // during render (not in the effect below) per React's "adjusting state
  // when a prop changes" pattern, since it's not itself a setState-in-effect.
